...
PLUGIN = {
    "name": "browser_setup",
    "description": (
        "Use a dedicated browser profile for approved developer-console setup. "
        "JARVIS can open and inspect supported sites and guide setup, while you "
        "enter authentication secrets directly into the target plugin setting. "
        "Separate from self-coding and voice."
    ),
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "action": {
                "type": "STRING",
                "description": "Action: open or inspect an approved developer site.",
                "enum": ["open", "inspect"],
            },
            "url": {
                "type": "STRING",
                "description": "Approved HTTPS developer-console URL.",
            },
        },
        "required": ["action"],
    },
...
PLUGIN_SETTINGS = {
    ...
    "fields": [
        ...
        {"key": "headless", "label": "3. SHOW BROWSER WINDOW", "type": "toggle", "default": False, "description": "OFF hides the automation window; ON lets you watch and interact with setup."},
        {"key": "profile_dir", "label": "4. AUTOMATION PROFILE FOLDER", "type": "text", "default": "", "placeholder": "Leave blank for Mark-LIII/config/browser_profile", "description": "Stores browser cookies locally in the dedicated automation profile."},
    ],
}
...
def _inspect(page) -> str:
    title = _redact(page.title())
    url = _redact(page.url)
    body = _redact(page.locator("body").inner_text(timeout=10000))[:14000]
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    if action not in {"open", "inspect"}:
        return "Use the browser_setup plugin for approved site navigation or inspection."
    try:
